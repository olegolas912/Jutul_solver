...
function simulator_storage(model; state0 = nothing,
    ...
    #-------------------------------------------------------------------------------------------------------------------------------------------------
    history_size = 5
    storage[:my_history] = Vector{Dict{Symbol,Any}}(undef, 0)
    storage[:history_size] = history_size
    #-------------------------------------------------------------------------------------------------------------------------------------------------
...
function solve_timestep!(sim, dT, forces, max_its, config;
    ...
    while !done
        ...
        if ok
            ...
            t_local += dt
#-------------------------------------------------------------------------------------------------------------------------------------------------
            current_state = get_output_state(sim.storage, sim.model)
            update_my_history!(sim.storage, current_state)
#-------------------------------------------------------------------------------------------------------------------------------------------------            
            if t_local >= dT
                # Onto the next one
                done = true
...
#-------------------------------------------------------------------------------------------------------------------------------------------------
function update_my_history!(storage::JutulStorage, st::Dict{Symbol,Any})
    hist = storage[:my_history]
    max_size = storage[:history_size]
    push!(hist, Dict(
        :pressure => st[:pressure],
        :s        => st[:s],
    ))
    if length(hist) > max_size
        popfirst!(hist)
    end
    storage[:my_history] = hist
end
#-------------------------------------------------------------------------------------------------------------------------------------------------
