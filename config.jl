...
Negative values disable output. The interpretation of this number is subject to change.")
    ...
    # Tolerances
    add_option!(cfg, :tolerances, set_default_tolerances(sim, 
    tol = nonlinear_tolerance), "Tolerances used for convergence criterions.")
    ...
    add_option!(cfg, :output_substates, true, "Store substates (between report steps) as field on each state.", types = Bool)
